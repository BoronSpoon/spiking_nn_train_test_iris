...
# initialize layers
fc1 = nn.Linear(num_inputs, num_hidden).to(device)
lif1 = snn.Leaky(beta=beta, reset_mechanism = "zero").to(device)
fc2 = nn.Linear(num_hidden, num_hidden).to(device)
lif2 = snn.Leaky(beta=beta, reset_mechanism = "zero").to(device)
fc3 = nn.Linear(num_hidden, num_outputs).to(device)
lif3 = snn.Leaky(beta=beta, reset_mechanism = "zero").to(device)
for layer in [fc1, lif1, fc2, lif2, fc3, lif3]:
    for param in layer.parameters():
        param.requires_grad = False
weights_len = num_inputs*num_hidden + num_hidden*num_hidden + num_hidden*num_outputs
...
def feed_forward(ga_instance, weights, solution_idx):
    ...
    for step in range(num_steps):
        cur1 = fc1(spk_in[step])/10 # post-synaptic current <-- spk_in x weight
    ...
    cur2_rec = torch.stack(cur2_rec)
    cur3_rec = torch.stack(cur3_rec)
    ...
    loss = float(loss_fn(spk3_rec, target))
    accuracy = float(acc_fn(spk3_rec, target))
